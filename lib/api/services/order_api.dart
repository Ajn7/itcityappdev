...
class OrderApi {
  ...
  Future<OrderStatusNew> createOrder(Order order) async {
    String customerJson =
        '{"user_id":"${order.customerId}","purchase_id":"${order.purchaseId}","total_amnt":"${order.totalAmount}","cur":"${order.currency}"}';
    print(customerJson);
    Response response = await _newApiClient.invokeAPI('$_createOrderPath', 'POST', customerJson);
    print(response.body.toString());
    return OrderStatusNew.fromJson(jsonDecode(response.body)['data']);
  }

  Future createPurchase(
      String? userId, double subtotal, String? currency) async {
    Response response = await _newApiClient.invokeAPI(
        _createPurchasePath +
            "?user_id=$userId&product_sub_total=$subtotal&cur=$currency",
        'POST_',
        {});
  ...
  Future<ProductOrderDetails> getPurchaseProductDetailsByOrderId(
      int? orderId) async {
