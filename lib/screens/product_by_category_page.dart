...
class _ProductListByCategoryState extends State<ProductListByCategory> {
  ...
  Widget build(BuildContext context) {
    print(
        'product list from selected cate: $products and lenght:${products.length}');
    return BlocListener<CartBloc, CartState>(
      listener: (context, state) {
        if (state is AddProductToCartLoadingState) {
          ...
          if (BlocProvider.of<CartBloc>(context).page!.compareTo('cartpage') ==
              1) {
